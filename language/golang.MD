...
- 类型比较
  - 不同类型不能比较
  - 不可比较类型不能比较  slice、map、function
  - 接口比较，一个接口覆盖另一个接口的所有方法，且不是同一种不可比较类型
- 线程安全的类型
  channel、sync.Map
...
```
make & new
- make  为slice、map、channel分配内存并初始化，返回值
- new  为任意类型分配内存，返回指针
...
- context  解决goroutine之间退出通知、元数据传递
  - 结构
