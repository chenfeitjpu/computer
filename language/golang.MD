...
变量 & 常量
- 变量转换时允许溢出
- 常量转换时不允许溢出，无类型常量可直接转换为其它类型，常量不能取值
...
- 类型比较
  ...
  - 接口比较，一个接口覆盖另一个接口的所有方法但实际类型不是同一种不可比较类型
- 线程安全的类型
  channel、sync.Map
```
...
make & new
- make  为slice、map、channel分配内存并初始化为非零值，返回的值
- new  为任意类型分配内存并初始化为零值，返回的是指针
...
- 实现
  ...
  - 并发标记
    - 对象标记
      - 分批扫描Goroutines栈帧，标记栈局部)变量为黑色，标记栈局部变量引用的堆对象为灰色，GC期间，任何在栈上新创建的对象，均为黑色
      - 标记其他根对象为灰色
        - 全局变量
        - 寄存器中的变量
      - 递归处理灰色对象
