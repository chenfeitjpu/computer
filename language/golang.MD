...
- 注意事项
  ...
  - 高效拼接
    - strings.Builder  构建字符串  String()时不会另外申请内存存储字节数组
    - bytes.Buffer  通用字节缓冲区  String()时会另外申请内存存储字节数组
  - 获取长度
...
- 扩容
  - 1.18以前
    - 如果新长度比原有容量两倍大，那么扩容后容量大小为新长度
    - 如果原有slice容量小于1024，那么就扩容为原来的2倍
    - 如果原有slice容量大于等于1024，那么每次扩容就扩为原来的1.25倍
  - 1.18及以后
    - 如果新长度比原有容量两倍大，那么扩容后容量大小为新长度
    - 如果原有slice容量小于256，那么就扩容为原来的2倍
    - 如果原有slice容量大于等于256，那么每次扩容就扩为原来的1.25倍+3/4*256
...
- 结构
  ...
  type bmap struct {
      tophash  [8]uint8   // 存储每个 key 的哈希值的高 8 位
      keys     [8]keytype   // 后面是 8 个 key
      values   [8]valuetype  // 然后是 8 个 value
      overflow uintptr  // 最后是指向溢出桶的指针
  }
- 查找过程
  - 计算key的hash值，通过最后的B位来确定桶号，通过前8位快速确定是在这个桶的哪个位置，对比key是否匹配
...
sync.map
- 实现  空间换时间，通过两个map(read和dirty)，实现读写分离，read无锁访问，dirty加锁访问，entry使用atomic操作
- 结构
  // sync/map.go
  type Map struct {
...
- 操作
  ...
  - 删除
    - key在ready中，将entry标记为nil(已删除直接跳过)
    - 如果read中没有，并且dirty有新数据，先上锁，双重检查，从dirty中直接删掉这key 
  - 遍历
...
- 注意事项
  - 对于T类型的接口，可以使用T类型的方法，调用*T类型的方法可能会报panic
  - 对于*T类型的接口，可以使用T和*T类型的方法
- 类型转换T(value)&类型断言value.(T)
  - 确定时机  类型转换是在编译期确定的，类型断言是运行期确定的
  - 使用场景  类型转换解决不同类型间的转换问题，类型断言把接口还原成具体类型
  - 底层实现  类型转换通常是简单的内存重新解释或者数据格式调整，类型断言需要检查接口的底层类型信息
...
make & new
- make  为slice、map、channel分配内存并初始化，返回的是值
...
- 概念
  ...
  - mcache  每个P持有一份的内存缓存，访问时无锁，包含68*2个mspan集合
...
- 并发问题
  - 漏标
    一个已经扫描完成的黑对象，指向了一个白色对象，且该白色对象上游没有灰色对象，白色对象会被回收
...
CSP(通信顺序进程)并发编程模型，它的核心思想是通过通信来共享内存，不要通过共享内存来通信。
- 结构
  // runtime/chan.go
  type hchan struct {
...
- 操作
  |       操作        |    正常channel   |     nil的channel   | close的channel |
  |:----------------:|:----------------:|:-----------------:| :-------------: |
  |    读 <-ch       | 成功或阻塞         | 阻塞               |    读到零值     |
  |    写 ch<-       | 成功或阻塞         | 阻塞               |      panic      |
...
- 与锁的区别
  ...
  - 性能开销
    ...
    - 锁通常具有较高的性能开销，因为它们需要线程同步和进行上下文切换等操作
...
- 概念
  - G  goroutine，要运行的协程
  - M  machine，os线程，真正执行指令
  - P  processor，调度器，持有本地队列和运行时资源
- 初始化
...
- 调度时机
  - 主动让渡  channel阻塞、select阻塞、锁、定时器
  - 阻塞让渡  g在执行过程中所依赖的外部条件没有达成，需要进入阻塞等待的状态
  - 抢占让渡  针对G运行时间太长(一般是10ms)的情况，直接发送信号给M，在安全点打断执行，把G放回runnable队列
