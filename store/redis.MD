...
- 内部实现
  ...
  - 扩容
    - sds 长度小于1MB，那么最后的扩容是按照翻倍扩容来执行的，即2倍的newlen
    - sds 长度超过1MB，那么最后的扩容长度应该是newlen+1MB
- 应用
  缓存、计数、分布式锁
...
- rehash
  ...
  - 实现
    - 插入  一律会被保存到ht[1]里面
    - 查询  先在ht[0]里面进行查找，如果没找到的话，就会继续到ht[1]里面进行查找
    - 遍历  新旧桶同时查询，然后合并数据进行去重
...
- 实现
  ...
  - 缓冲区的生成时机
    - 主服务器启动
      - 设置master_repl_offset为0
    - 第一个从服务器连接主服务器
      - 主服务生成repl_backlog_buffer
    - 从服务器连接主服务器
      - 主服务器为每个从服务器生成replication buffer
  - 同步
    - 全量同步
      - 第一次同步，建立链接、协商同步
...
- 实现
  ...
  - 对key的CRC16值取模2^14计算出哈希槽，根据哈希槽找到节点
- 重定向
  - Moved 重定向
  ...
  - ASK 重定向
    一般发生于集群伸缩的时候。集群伸缩会导致槽迁移。
    - 当我们访问源节点，对于槽中不存在的Key，会返回ASK重定向以告知客户端该当前槽迁入的目的节点地址信息
    - 当我们访问目的节点，每次访问属于迁移槽的Key时，没有先发送ASKING命令，其均会返回MOVED重定向。先发送ASKING命令，该Key迁入的话，则直接返回结果；如果该Key未迁入，则直接返回null
- 故障转移
  - 发现故障
    - 某个节点认为另一个节点不可用，将其标记为主观下线
    - 集群内多个节点都认为该节点不可用，将其标记为客观下线，如果是持有槽的主节点故障，需要为该节点进行故障转移
  - 经过多节点投票，拥有hash槽的主节点可以投票，选举新主节点
  - 所有节点更新集群拓扑
```
...
[浅谈Redis Cluster 集群的MOVED、ASK重定向](https://zhuanlan.zhihu.com/p/655514905)