...
condition filtering(扇出值)

**调节成本常数**
  - server层(mysql.server_cos)
  - 存储引擎层(mysql.engine_cost)
...
可通过innodb_stats_persistent变量设置系统的存储方式  
可通过STATS_PERSISTENT属性指明表的存储方式
...
IN子查询优化  
  - 将子查询转换为semi-join
    ...
    - Semi-join Materialization execution strategy（物化连接）
