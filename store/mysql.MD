...
- [分析的查询语句是什么](#分析的查询语句是什么)
- [优化的具体过程](#优化的具体过程)
- [因优化过程文本太多而丢弃的文本字节大小，值为0时表示并没有丢弃](#因优化过程文本太多而丢弃的文本字节大小值为0时表示并没有丢弃)
- [权限字段](#权限字段)
  - [事务](#事务)
    - [特性](#特性)
    - [状态](#状态)
...
### 执行过程 ###
#### 查询 ####
![avatar](./image/mysql/../../images/mysql/flow_query.webp)
...
#### 比较 ####  
`\>, \>=, \<, \<=, =, !=, between and, in, not in, is null, is not null, like, not like`
...
区间记录计算
- index dive 直接访问索引对应的B+树来计算某个范围区间对应的索引记录条数
- 索引统计 单点区间超过eq_range_index_dive_limit时使用
```
...
更新  
- 定期更新 通过系统变量`innodb_stats_auto_recalc`设置
...
![avatar](./images/mysql/transaction-state.awebp)
...
InnoDB表对主键的生成策略：优先使用用户自定义主键作为主，如果用户没有定义主键，则选取一个Unique键作为主键，如果表中连Unique键都没有定义的话，则InnoDB会为表默认添加一个名为row_id的隐藏列作为主键  
InnoDB存储引擎会为每条记录都添加 transaction_id 和 roll_pointer 这两个列，但是 row_id 是可选的
...
- `create view 视图名[(列名,列名)] as select` 创建视图
- `drop view 视图名` 删除视图
...
*结构*  
![avatar](./images/mysql/extent_xdes_entry.webp)
```
...
*结构*  
![avatar](./images/mysql/segment_inode_entry.webp)
```
- Segment ID 段号  
...
#### 独立表空间 ####
![avatar](./images/mysql/table_space_per.webp)

**FSP_HDR**  
![avatar](./images/mysql/table_space_page_fsp_hdr.webp)

**INODE**  
![avatar](./images/msyql/../mysql/table_space_page_inode.webp)

#### 系统表空间 ####
![avatar](./images/mysql/table_space_system.webp)

**XDES**  
![avatar](./images/mysql/table_space_page_xdes.webp)

#### 总览 ####
![avatar](./images/mysql/table_space.webp)
...
***内部组成***  
![avatar](./images/mysql/innodb-buffer-pool.awebp)
...
![avatar](./images/mysql/redo-log-mini-transaction.awebp)
...
**flush链表中的LSN**  
- oldest_modification  
  某个页面被加载到Buffer Pool后进行第一次修改，那么就将修改该页面的mtr开始时对应的lsn值写入这个属性  

- newest_modification
  每修改一次页面，都会将修改该页面的mtr结束时对应的lsn值写入这个属性
...
![avatar](./images/mysql/redo-log-file-lsn.awebp)
...
***undo页面链表***  
![avatar](./images/mysql/undo-log-list.awebp)

***undo日志具体写入过程***  
**Undo Log Segment Header**  
![avatar](./images/mysql/undo-log-segment-header.awebp)
...
MySQL设计了一个称之为Rollback Segment Header的页面，在这个页面中存放了各个Undo页面链表的frist undo page的页号

**多个回滚段**  
MySQL默认定义了128个回滚段  
![avatar](./images/mysql/undo-log-rollback-segment.awebp)
